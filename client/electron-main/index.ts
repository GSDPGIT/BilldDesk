...
import {
  attachSilentBehavior,
  initSilent,
  isSilent,
  silentWindowOptions,
} from './silent-mode';
import { registerSilentIpc } from './silent-ipc';
...
function main() {
  const mainWindow = new BrowserWindow({
    ...
    frame: false,
    // 静默模式下注入隐藏 / 不在任务栏 / 不可关闭
    ...silentWindowOptions(),
  });
  attachSilentBehavior(mainWindow);
...
app.on('window-all-closed', () => {
  // 静默模式下保持常驻；silent-mode.ts 也注册了同事件并 preventDefault
  if (isSilent()) return;
  app.quit();
  windowMap.clear();
});
